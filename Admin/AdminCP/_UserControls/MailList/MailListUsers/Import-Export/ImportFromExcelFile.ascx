<%@ Control Language="C#" AutoEventWireup="true" CodeFile="ImportFromExcelFile.ascx.cs" Inherits="AdminCP__UserControls_MailList_MailListUsers_Import_Export_ImportFromExcelFile" %>
<table class="MainTable" cellspacing="0" cellpadding="0" width="100%" border="0">
        ...
        <tr>
            <td class="text">
                <asp:Label  Text="<%$ Resources:MailListAdmin,ImportMails_File %>"  runat="server" /><span class="RequiredField"><asp:Label  runat="server"
                    Text="*" /></span>
            </td>
            <td class="Control">
                <asp:FileUpload ID="fuFile" runat="server" CssClass="Controls" /><a href="/AdminCP/MailList/MailListUsers/Import-Export/SampleFiles/mails-sample.xls" title="<%= Resources.MailListAdmin.FileSample %>" target="_blank"><%= Resources.MailListAdmin.FileSample %></a>
                ...
                <asp:RegularExpressionValidator ID="RFV" runat="server" ControlToValidate="fuFile"
                    ValidationGroup="MailListUsers" ErrorMessage="<%$ Resources:MailListAdmin,ImportMails_FileValidation %>" ValidationExpression="(.*\.([xX][Ll][sS])$|.*\.([xX][Ll][sS][xX])$)"></asp:RegularExpressionValidator>
            </td>
        </tr>
        <tr>
