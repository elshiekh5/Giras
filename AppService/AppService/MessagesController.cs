...
namespace AppService
{
    ...
    public class MessagesController
    {
        ...
        public static bool ContactUS(ContactUsModel model, out string resultMessage)
        {
            ...
            msg.Mobile = model.Mobile;
            mailBody += string.Format(rowTemplate, DynamicResource.GetMessageModuleText(currentMessageModule, "Mobile"), msg.Mobile);
        ...
        //------------------------------------------------------------------------------------
        public static bool TrainingReginster(TrainingRegModel model, out string resultMessage)
        {
            MessagesModuleOptions currentMessageModule = MessagesModuleOptions.GetType(model.ModuleTypeID);
            //Preparing admin notification email
            string mailBody = "<table style='width:auto; direction:" + DynamicResource.GetText("Lang", "Dir") + "'>";
            MessagesEntity msg = new MessagesEntity();

            //-------------------------------------
            msg.ModuleTypeID = model.ModuleTypeID;
            //-------------------------------------------------------------------------- 
            msg.ToItemID = model.RelatedItemID;
            msg.Name = model.FirstName + " "+ model.LastName;
            mailBody += string.Format(rowTemplate, DynamicResource.GetMessageModuleText(currentMessageModule, "Name"), msg.Name);
            msg.Mobile = model.Mobile;
            mailBody += string.Format(rowTemplate, DynamicResource.GetMessageModuleText(currentMessageModule, "Mobile"), msg.Mobile);
            msg.Tel = model.Tel;
            mailBody += string.Format(rowTemplate, DynamicResource.GetMessageModuleText(currentMessageModule, "Phone"), msg.Tel);
            msg.EMail = model.Email;
            mailBody += string.Format(rowTemplate, DynamicResource.GetMessageModuleText(currentMessageModule, "Email"), msg.EMail);
            //-------------------------------------
            msg.LangID = SiteSettings.GetCurrentLanguage();
